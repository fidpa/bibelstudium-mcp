...
- **Querverweise** (`bible_crossrefs`): Treasury of Scripture Knowledge (erweitert, OpenBible.info), nach Community-Stimmen gewichtet, mit deutschem Zieltext; mehrversige Ziele zusätzlich versweise aufgeschlüsselt (`verse_einzeln`). Reicht ein Ziel über mehr als vier Verse oder über eine Kapitelgrenze, steht nur sein Anfang im Wortlaut, und `abschnitt_gekuerzt` nennt je Verweis, wie viele Verse geliefert sind. In einer Ausgabe mit Wortlaut-Grenze tragen die bestbewerteten Verweise ihren Text, die übrigen nur `stelle` und `votes`; die Zahl der Verweise bleibt gleich. Wie viele Verweise es zu der Stelle überhaupt gibt, nennt `gesamt`, unabhängig von `limit`
