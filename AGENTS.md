...
- **Belege statt Behauptungen.** Jede Aussage über den Text gegen echte Daten
  prüfen (SQL gegen `data/bible.db` oder ein frischer stdio-Aufruf des Servers), niemals aus dem Gedächtnis. Bei Fragen zum Grundtext die Edition tatsächlich
  abrufen.
- **Eine laufende MCP-Server-Instanz in einer Agenten-Sitzung ist veraltet**: Codeänderungen wirken erst nach Neustart. Änderungen immer über einen frischen
...
- **Kein Em-Dash (`—`) in deutschem Text.** Im Deutschen ist er das falsche
  Zeichen; je nach Satzbau gehört dort ein Doppelpunkt, ein Punkt, ein Komma
  oder ein Semikolon hin, und wo wirklich ein Gedankenstrich nötig ist, der
  Halbgeviertstrich `–` mit Leerzeichen. Gilt für Dokumentation **und** für
  deutsche Ausgabetexte in `server.ts` (`hinweis`, `warnung`, `lesehinweis`,
  Fehlermeldungen, Prompts). **Nicht** betroffen: englische Kommentare und die
  englischen Tool-`description`s, dort ist der Em-Dash korrekt, sowie der
  Platzhalter `"—"` für „kein Wert", der ein Symbol ist und kein Satzzeichen.
...
Priorität steht in `resolveEdition`, im Routing und im `hinweis` jeder Antwort, bei Änderungen konsistent halten).
...
- **`mcpb/manifest.json` ist die dritte Doku-Stelle, und die einzige, die ein
  ...
  Minuten" und „beim Installieren über den Dateiauswahldialog ausgewählt", beides zu dem Zeitpunkt falsch, und dem Nutzer angezeigt, während README und
...
- **Tool-Beschreibungen steuern, ob überhaupt aufgerufen wird.** `bible_lookup`
  ...
  Werkzeug in der Praxis übergangen wird, zuerst seine `description` prüfen, nicht die Ausgabe.
