...
const CALLS = [
  ...
  // Die Wortlaut-Grenze zweier Ausgaben (`verseMax` in translations.ts). Neue
  // Fälle werden ANGEHÄNGT, nie eingeschoben: Die Ergebnisse werden weiter
  // unten positionsweise zerlegt, und ein Einschub in der Mitte verschöbe jeden
  // Namen danach, lautlos.
  //
  // Drei Werkzeuge, jedes mit seiner eigenen Gestalt der Kürzung, dazu die
  // Gegenprobe in einer Ausgabe ohne Grenze. Geprüft wird an der Grenze und
  // einen Vers darüber; die Zahl selbst steht in keiner Zusicherung, sie wird
  // aus `gekuerzt.verse_max` derselben Antwort genommen. Sonst kostete eine
  // geänderte Zusage nicht eine Registry-Zeile, sondern ein Dutzend Literale.
  ["bible_lookup", { book: "Psalter", chapter: 66, translation: "SLT" }], // genau 20 Verse
  ["bible_lookup", { book: "Psalter", chapter: 9, translation: "SLT" }], // 21 Verse, einer zu viel
  ["bible_lookup", { book: "Psalter", chapter: 119, translation: "SLT" }],
  ["bible_lookup", { book: "Psalter", chapter: 119, translation: "SCH" }], // gilt auch für die 1951er
  ["bible_lookup", { book: "Psalter", chapter: 119, translation: "LUT" }],
  ["bible_lookup", { book: "Psalter", chapter: 119, translation: "ELB" }],
  ["bible_lookup", { book: "Psalter", chapter: 119, translation: "MB" }],
  ["bible_search", { query: "Gott", translation: "SCH", limit: 50 }],
  ["bible_search", { query: "Gott", translation: "SCH", limit: 10 }], // unter der Grenze
  ["bible_search", { query: "Gott", translation: "LUT", limit: 50 }],
  ["bible_crossrefs", { book: "Joh", chapter: 3, verse: 16, limit: 30, translation: "SCH" }],
  ["bible_crossrefs", { book: "Joh", chapter: 3, verse: 16, limit: 30, translation: "LUT" }],
...
const RESOURCE_CALLS = [
  ...
  // Die Wortlaut-Grenze an der Ressource. Angehängt und nicht bei den übrigen
  // gelingenden Fällen eingeschoben, weil ein Einschub jeden Negativfall
  // darunter verschöbe. Der Grenzsatz muss zeichengleich mit dem des Werkzeugs
  // sein, geprüft gegen dessen Antwort und nicht gegen ein Literal.
  "bible://kapitel/SCH/Psalter/119",
  "bible://kapitel/LUT/Psalter/119",
...
const [
  ...
  resGrenzeSch,
  resGrenzeLut,
] = resourceResults as ResourceResult[] & { length: 22 };
const [
  origVerse999,
  origChap999,
  xrefVerse999,
  ...
  grenzeXrefSch,
  grenzeXrefLut,
] = results as ToolResult[] & { length: 47 };
...
// --- Wortlaut-Grenze ---------------------------------------------------------
// Die Grenze hängt an der Ausgabe (`verseMax` in translations.ts) und nicht am
// Server, also wird sie in beide Richtungen geprüft: an den zwei begrenzten
// Ausgaben, dass sie greift und es sagt, und an allen drei gemeinfreien, dass
// sich nichts geändert hat.
//
// Nirgends steht die Zahl selbst. Jede Erwartung wird aus `gekuerzt.verse_max`
// derselben Antwort gerechnet, denn die Zahl ist eine Zusage und keine
// Eigenschaft des Codes: Sie zu ändern soll eine Registry-Zeile kosten und
// nicht ein Dutzend Testliterale.
{
  console.log("Wortlaut-Grenze");
  ...
  // Nennung nennt, und aus derselben Registry.
